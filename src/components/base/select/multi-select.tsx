      ...
      {!isSelectionEmpty &&
        comboBoxContext?.selectedItems?.items?.map(value => (
          ...
          >
            ...
            <p
              className={cx(
                "truncate text-sm font-medium whitespace-nowrap text-secondary select-none",
                value?.avatarUrl && "ml-1.25"
              )}
            >
              {value?.label}
            </p>
      ...
      >
        ...
        {shortcut && (
          ...
          >
            <span
              className={cx(
                "pointer-events-none rounded px-1 py-px text-xs font-medium text-quaternary ring-1 ring-ws-border-primary select-none ring-inset",
  ...
  return (
    <AriaGroup
      ...
      className={({ isFocusWithin, isDisabled }) =>
        cx(
          "relative flex w-full items-center gap-2 rounded-lg bg-primary shadow-xs ring-1 ring-ws-border-primary outline-hidden transition duration-100 ease-linear ring-inset pr-10!",
